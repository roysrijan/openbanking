    ...
    async function getBalance(){
      ...
      useEffect(()=>{
        async function get(){
            ...
            let accounts = await AsyncStorage.getItem('accounts');
            accounts = JSON.parse(accounts);
            if(accounts?.includes('ReadTransactionsDetail')) setPermission(true);
            let isPaid = await AsyncStorage.getItem('payment');
            isPaid = JSON.parse(isPaid);
            console.log(isPaid)
            if(isPaid) setTransactions(prev=>[...prev,...[
              {
                title: 'Buddha Painting',
                subTitle: 'Religious Panel Painting',
                img: require('../assets/deal4.jpeg'),
                price: '-99'
              }
            ]]);
            else setTransactions(transactions.slice(0,1));
        }
        get();
      },[isFocused]);
            ...
            <View style={styles.profileInfoWrapper}>
              <View style={styles.profilePicContainer}>
                ...
                <View style={styles.profileForm}>
                  ...
                  <Text style={styles.profileName}>My Transactions</Text>  
                  <Text style={styles.profileName}></Text>  
                  {permission && transactions?.map((ele,index)=>(
                        <ListItem
                            key={index}
                        ...
                        >
                            ...
                            <Text style={{color: 'red'}}>{ele.price}</Text>
                        </ListItem>))}
                  {accounts.length === 0 &&
                    <>
                  ...
                  {!permission && accounts.length !== 0 &&
                    <>
                        <Text style={{marginLeft: 10}}>You don't have access to ReadTranactionsDetail.</Text>
                        <Text style={{marginLeft: 100}}>Please Subscribe!!</Text>
                    </>
                  }
                  </View>
